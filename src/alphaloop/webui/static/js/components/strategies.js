...
export async function render(el) {
  ...
  function renderDetailShell() {
    ...
    const lockedFromBacktest = strat.source === 'backtest_runner';
    ...
    const toolsLockBanner = lockedFromBacktest ? `
      <div style="display:flex;align-items:center;gap:8px;padding:10px 14px;background:var(--bg2);border:1px solid var(--border);border-radius:var(--r-xl);margin-bottom:12px;font-size:var(--fs-sm);color:var(--muted)">
        \uD83D\uDD12 Safety filters are locked — this strategy was generated by SeedLab and its filters are part of the backtest record.
      </div>` : '';
    const toolsHtml = toolsLockBanner + DETAIL_TOOL_GROUPS.map(group => {
            ...
            ${group.tools.map(name => {
              ...
              return `
                <label class="sd-tool-item">
                  ...
                  <input id="${detailId('tool', name)}" type="checkbox" ${on ? 'checked' : ''} ${lockedFromBacktest ? 'disabled' : ''}>
                </label>
              `;
            }).join('')}
          </div>
    ...
    const signalHtml = `
      ...
      ${lockedFromBacktest ? `
      <div style="display:flex;align-items:center;gap:8px;padding:10px 14px;background:var(--bg2);border:1px solid var(--border);border-radius:var(--r-xl);margin-bottom:12px;font-size:var(--fs-sm);color:var(--muted)">
        \uD83D\uDD12 Entry signals are locked — this strategy was generated by SeedLab and its signals are part of the backtest record.
      </div>` : ''}
      <div class="sd-section">
        ...
        <div class="sd-tool-group">
          ${SIGNAL_SOURCES.map(src => `
            <label class="sd-tool-item" style="${lockedFromBacktest ? 'opacity:0.6;pointer-events:none' : ''}">
              <div>
                <div class="sd-tool-name">${src.label}</div>
                <div class="sd-tool-key">${src.value}</div>
              </div>
              <input id="signal-src-${src.value}" type="checkbox" ${activeSourceSet.has(src.value) ? 'checked' : ''} ${lockedFromBacktest ? 'disabled' : ''} style="accent-color:var(--blue)">
            </label>
          `).join('')}
        </div>
      </div>
      <div class="sd-section">
        ...
        <div class="sd-mode-bar">
          ${SIGNAL_LOGIC_OPTIONS.map(opt => `
            <button class="sd-mode-btn signal-logic-btn ${signalLogic === opt ? 'active' : ''} ${lockedFromBacktest ? 'locked-btn' : ''}" data-logic="${opt}" ${lockedFromBacktest ? 'disabled' : ''}>${opt}</button>
          `).join('')}
        </div>
      </div>
      ` : ''}
