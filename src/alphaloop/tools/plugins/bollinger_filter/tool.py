...
# Threshold: block BUY when near upper band, SELL when near lower band
_BUY_MAX_PCT_B  = 0.7
_SELL_MIN_PCT_B = 0.3
...
class BollingerFilter(BaseTool):
    ...
    async def run(self, context) -> ToolResult:
        ...
        if direction == "BUY":
            if pct_b < _BUY_MAX_PCT_B:
                return ToolResult(
                    passed=True,
                    reason=(
                        f"BB %B {pct_b:.3f} < {_BUY_MAX_PCT_B} — "
            ...
            return ToolResult(
                passed=False,
                reason=(
                    f"BUY blocked: BB %B {pct_b:.3f} >= {_BUY_MAX_PCT_B} — "
        ...
        if direction == "SELL":
            if pct_b > _SELL_MIN_PCT_B:
                return ToolResult(
                    passed=True,
                    reason=(
                        f"BB %B {pct_b:.3f} > {_SELL_MIN_PCT_B} — "
            ...
            return ToolResult(
                passed=False,
                reason=(
                    f"SELL blocked: BB %B {pct_b:.3f} <= {_SELL_MIN_PCT_B} — "
    ...
    async def extract_features(self, context) -> FeatureResult:
        ...
        return FeatureResult(
            ...
            reference_thresholds={"buy_max_pct_b": 0.7, "sell_min_pct_b": 0.3},
